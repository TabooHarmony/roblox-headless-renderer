...
_ALLOWED_POSTPROCESS_FONTS = {
    ...
    "gothamssm": "GothamSSm",
    # Pixel faces must survive canonicalization: Press Start 2P advances 1em/glyph
    # in Roblox (suite Winner pair: 'Mission Completed!' pitch 54px at fit 54);
    # defaulting it to GothamSSm rendered it at the wrong face and fit entirely.
...
def _postprocess_node(node: dict[str, Any]) -> dict[str, Any]:
    ...
    # TextScaled is no longer forced on here: the converter keeps the model's own
    # value, and the renderer fits the font only when the label asks for it
    # (patch 0006). Fitting every label scaled text that Roblox renders at TextSize.
