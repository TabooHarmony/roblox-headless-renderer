...
# The bundled Press Start 2P TTF draws its glyphs at 0.5em (measured: 'M' ink
# 27px at nominal size 54); Roblox's binary fills the cell (studio Winner pair:
# 47px ink in the 54px cell). Scale the glyph draw so the ink matches Roblox
# while the 1em advance pitch stays unchanged.
_PIXEL_FONT_TTF_INK_EM = 0.5
_PIXEL_FONT_ROBLOX_INK_EM = 47.0 / 54.0
_PIXEL_FONT_GLYPH_SCALE = _PIXEL_FONT_ROBLOX_INK_EM / _PIXEL_FONT_TTF_INK_EM

def _build_fallback_fonts(typefaces: list[skia.Typeface], size: float) -> tuple[skia.Font, ...]:
    fonts: list[skia.Font] = []
    for tf in typefaces:
        f = roblox_font(tf, size)
...
def _is_pixel_font(font: skia.Font) -> bool:
    """True for pixel faces Roblox advances at exactly 1em per glyph.

    Press Start 2P is an 8x8 pixel font whose design advance is 1em. Skia reads
    the TTF's 0.56em advance, but Roblox (measured, suite Winner pair:
    'Mission Completed!' pitch 54px at fitted size 54) spaces it at 1em, which
    also drives TextScaled fitting. Same class of gap as the FredokaOne table.
    """
    try:
        return font.getTypeface().getFamilyName() in _PIXEL_FONT_ADVANCE_EM
    except Exception:
        return False


def _pixel_advance(font: skia.Font) -> float:
    return float(font.getSize()) * _PIXEL_FONT_ADVANCE_EM.get(
...
def _draw_run_text(canvas: skia.Canvas, run_text: str, x: float, baseline_y: float,
    ...
    if not _is_robux_run(run_text):
        if _is_pixel_font(font):
            # Per-char placement at the 1em pitch: drawString would space glyphs
            # at the TTF's 0.56em advance, half of Roblox's.
            step = _pixel_advance(font)
            k = _PIXEL_FONT_GLYPH_SCALE
            if abs(k - 1.0) > 1e-6:
                # The bundled TTF's glyphs are half-size inside the em; scale
                # each glyph around its baseline so the ink fills the cell
                # like Roblox's binary, keeping the 1em pitch.
                canvas.save()
                for i, ch in enumerate(run_text):
                    gx = x + i * step
                    canvas.translate(gx, baseline_y)
                    canvas.scale(k, k)
