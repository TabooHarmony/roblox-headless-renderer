...
def draw_background(
    ...
    # Legacy GuiObject border: the outer `size` pixels of the rect render in
    # the border colour and the background is inset inside them (Roblox draws
    # the border inside the object's rect; ScrollGui pair measured the 5px
    ...
    if border:
        ...
        if bsize > 0:
            # Roblox adds BorderSizePixel OUTSIDE Size: the border band wraps
            # the object rect (ScrollGui pair: 5px slate at rect - 5 .. rect),
            # and the background fills the inner Size area.
            bp = skia.Paint(AntiAlias=True, Color=skia.Color(
                int(bcolor[0]), int(bcolor[1]), int(bcolor[2]), 255))
            canvas.drawRect(skia.Rect.MakeXYWH(
                x - bsize, y - bsize, w + 2 * bsize, h + 2 * bsize), bp)
    ...
    rect = skia.Rect.MakeXYWH(bx, by, bw, bh)
