    ...
    for child in children:
        ...
        if cls == "UICorner":
            ...
            if fill_dir == "Vertical":
                ...
                grid["direction"] = "X"
            cell_size = _udim2_to_full(props.get("CellSize"))
            if cell_size is not None:
                grid["cellSize"] = cell_size
            cell_pad = _udim2_to_full(props.get("CellPadding"))
...
def flatten_node(raw: dict) -> dict:
    ...
    # ResampleMode: Default (0) is smooth filtering, Pixelated (1) keeps hard
    # pixel edges. Only the non-default value survives: the engine's sampling
    # default is smooth.
    resample_mode = _extract_enum_value(props.get("ResampleMode"))
    if resample_mode == "Pixelated":
        result["resampleMode"] = "Pixelated"
