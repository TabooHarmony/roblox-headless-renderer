...
| 0019-system-symbol-fallback.patch | symbols common in game UIs (✕ ✓ ★ ▶) are in no bundled or Roblox-shipped face and drew as empty boxes; Roblox takes them from the OS font fallback. The fallback list now ends with well-known system symbol fonts (Segoe UI Symbol, Apple Symbols, DejaVu/Noto), loaded through the FreeType manager like the bundled fonts (asking the platform font manager crashes skia-python on macOS); only characters every loaded face lacks change. `RHR_SYSTEM_FONT_FALLBACK=0` turns it off (the tests do). Written as a plain diff (no pristine-tree rebuild on the maintainer's Windows box yet) | text_fonts.py | test_symbol_fallback.py |
