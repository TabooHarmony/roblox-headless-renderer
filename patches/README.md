...
| 0019-system-symbol-fallback.patch | symbols common in game UIs (✕ ✓ ★ ▶) are in no bundled or Roblox-shipped face and drew as empty boxes; Roblox takes them from the OS font fallback. The fallback list now ends with the system faces that cover those symbol blocks; only characters every loaded face lacks change. `RHR_SYSTEM_FONT_FALLBACK=0` turns it off (the tests do). Written as a plain diff (no pristine-tree rebuild on the maintainer's Windows box yet) | text_fonts.py | test_symbol_fallback.py |
